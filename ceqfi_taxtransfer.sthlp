{smcl}
{* 8dec2016}{...}
{cmd:help ceqfi_taxtransfer} (beta version; please report bugs) {right:Rosie Li}
{hline}

{title:Title}

{p 4 11 2}
{hi:ceqfi} {hline 2} Calculates the measures of fiscal impoverishment (FI) and fiscal gains to the poor (FGP) from Higgins and Lustig (2016) by taxes and transfers. Graph the level of 
FI and FGP by value of povery line defined. Modified from {bf:ceqfi} (Higgins 2016).

{title:Syntax}

{p 8 11 2}
    {cmd:ceqfi_taxtransfer} {ifin} {weight} [{cmd:using} {it:filename}] [{cmd:,} {it:options}]{break}
...
{synopt :{opt graph}}Specify to produce FI or FGP graphs for specific fiscal interventions. Otherwise no graph is produced.{p_end}
{synopt :{opth graphl:ine(real)}}Specify the maximum value on the poverty line x-axis. (default is $10) The maximum value allowed is $100{p_end}
{synopt :{opth graphint:er(varlist)}}Specify the fiscal interventions to be graphed. The default is to produce graphs for all fiscal interventions{p_end}
...
{pstd} 
{cmd:ceqfi_taxtransfer} calculates the measures of fiscal impoverishment (FI) and fiscal gains to the poor (FGP) 
...
{pstd}
{cmd: ceqfi_taxtransfer} automatically converts local currency variables to PPP dollars, using the PPP conversion 
...
using {help svyset} since {cmd:ceqfi_taxtransfer} automatically uses the information specified using {help svyset}. 
...
{cmd: ceqfi_taxtransfer} provides the option of producing graphs for each of the FI or FGP indicator given an interval of values defined to be the poverty line. 
...
{pstd}
By default, {cmd: ceqfi_taxtransfer} does not allow income concept or fiscal intervention variables to have missing 
...
default, {cmd:ceqfi_taxtransfer} prints fiscal impoverishment results to a sheet titled "E5. Fisc. Impov." 
...
populated with results from {cmd:ceqfi_taxtransfer} can be automatically opened if the {opt open} option is 
...
{phang} {cmd:. ceqfi_taxtransfer [pw=w] using C:/Output_Tables.xlsx, hhid(hh_code) psu(psu_var) strata(stra_var) income(ym) taxes(personal_inc_tax property_tax) transfers(scholarship) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') country("Brazil") surveyyear("2008-2009") authors("Sean Higgins and Claudiney Pereira") baseyear(2005) graphl(6) graphinter(property_tax scholarship) open}{p_end}
