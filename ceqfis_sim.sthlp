{smcl}
{* 4jan2017}{...}
{cmd:help ceqfis_sim} (beta version; please report bugs) {right:Rosie Li}
{hline}

{title:Title}

{p 4 11 2}
{hi:ceqfi} {hline 2} Simulates the impact of fiscal impoverishment (FI) and fiscal gains to the poor (FGP) from Higgins and Lustig (2016) when taxes 
are raised to fund transfers or when transfers are defunded to issue tax returns. Modified from {bf:ceqfi} (Higgins 2016).

{title:Syntax}

{p 8 11 2}
    {cmd:ceqfis_sim} {ifin} {weight} [{cmd:using} {it:filename}] [{cmd:,} {it:options}]{break}
...
{synopt :{opth transfer(varname)}}The fiscal transfer subject to change{p_end}
{synopt :{opth transfers:cale(real)}}The direction and magnitude of the fiscal transfer. Can be positive or negative, but cannot be zero{p_end}
{synopt :{opth tax1(varname)}}Tax to be used to fund the transfer or to be subject to tax cut{p_end}
{synopt :{opth taxprop1(real)}}The proportion of changed budget relevant to tax1. Specified to be between 0 and 1{p_end}
{synopt :{opth tax2(varname)}}Tax to be used to fund the transfer or to be subject to tax cut, optional{p_end}
{synopt :{opth taxprop2(real)}}The proportion of changed budget relevant to tax2. Specified to be between 0 and 1{p_end}
{synopt :{opth tax3(varname)}}Tax to be used to fund the transfer or to be subject to tax cut, optional{p_end}
{synopt :{opth taxprop3(real)}}The proportion of changed budget relevant to tax3. Specified to be between 0 and 1{p_end}
...
{synopt :{opth sheetfi(string)}}Name of Excel sheet to write fiscal impoverishment results. Default is "Fisc. Impov."{p_end}
{synopt :{opth sheetfg(string)}}Name of Excel sheet to write fiscal gains to the poor results. Default is "Fisc. Gains to the Poor"{p_end}
...
{pstd} 
{cmd:ceqfis_sim} simulates the impact of fiscal policy changes on fiscal impoverishment (FI) and fiscal gains to the poor (FGP) 
...
{pstd}
The user specifies which transfer to fund or defund by using the {opth transfer(varname)} and 
specifies the policy change using the {opth transferscale(real)}. Positive sign denotes expanding the transfer program and negative sign indicates shrinking the program.
The magnitude refers to the scale of change. For example, 0.3 means every beneficiary receives 30% more transfers; -0.2 means transfer is reduced by 20% for each
recipient. The taxes designated to be involved in fiscal policy changes (up to three) are specified with {opth tax1(varname)} {opth tax2(varname)} {opth tax3(varname)}.
The program is designed such that the budget is balanced after the policy change. Hence, if transfer funding is to increase, the taxes will increase to provide
funding for the expansion. On the other hand, if the transfer is to scale back, the taxes will be reduced using the "extra" revenue. The user decides how the budget 
is allocated among taxes using {opth taxprop1(real)} {opth taxprop2(real)} and {opth taxprop3(real)}. These options have to be between 0 and 1 as they point to what
proportion of the changed funding influences a particular tax. If more than one tax is specified, then the user has to specify the proportion options corresponding to
the taxes. 
...
{pstd}
{cmd: ceqfis_sim} automatically converts local currency variables to PPP dollars, using the PPP conversion 
...
using {help svyset} since {cmd:ceqfis_sim} automatically uses the information specified using {help svyset}. 
...
{pstd}
The FI and FGP results change as the poverty line used changes.
{cmd: ceqfis_sim} provides the option of producing graphs for each of the FI or FGP indicator given an interval of values defined to be the poverty line. 
The corresponding level of the FI or FGP indicator (total fiscal impoverishment for example) is plottd
against the x-axis interval denoting the poverty line used. The user can choose the maximum value of poverty line "m" by specifying 
{opt graphl:ine(m)}. The x-axis poverty line interval will then be [0,m]. The user can also choose what fiscal interventions to be graphed
by using the option {opth graphint:er(varlist)}. The variables supplied to this options need to be within the variables 
that are fed into the {opth taxes(varlist)} or {opth transfers(varlist)} option.

{pstd}
By default, {cmd: ceqfis_sim} does not allow income concept or fiscal intervention variables to have missing 
...
default, {cmd:ceqfis_sim} prints fiscal impoverishment results to a sheet titled "E5. Fisc. Impov." 
...
populated with results from {cmd:ceqfis_sim} can be automatically opened if the {opt open} option is 
...
{phang} {cmd:. ceqfis_sim [pw=w] using C:/Output_Tables.xlsx, hhid(hh_code) psu(psu_var) strata(stra_var) income(ym) tax1(personal_inc_tax) taxprop1(0.6) tax2(property_tax) taxprop2(0.4) transfers(scholarship) transferscale(-0.3) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') country("Brazil") surveyyear("2008-2009") authors("Sean Higgins and Claudiney Pereira") baseyear(2005) graphl(6) graphinter(property_tax scholarship) open}{p_end}

{pstd}Household-level data (each observation is a household){p_end}
{phang} {cmd:. ceqfis_sim [pw=w] using C:/Output_Tables.xlsx, hsize(members) psu(psu_var) strata(stra_var) income(ym) tax1(personal_inc_tax) taxprop1(0.6) tax2(property_tax) taxprop2(0.4) transfers(scholarship) transferscale(0.2) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') country("Brazil") surveyyear("2008-2009") authors("Sean Higgins and Claudiney Pereira") baseyear(2005) graphl(6) graphinter(property_tax scholarship) open}{p_end}
